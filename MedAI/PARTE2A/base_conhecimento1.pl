...
caminhoMaisPassageiros(X,Y,Caminho, V):- retractall(ocupacao(_,_)), listaCaminhos(X,Y,Lista), 
gerarOcupacoesCaminhos(Lista), maisPassageiros(Caminho, V).

maisPassageiros(Caminho, V):- ocupacao(Caminho, V), \+ (ocupacao(_, V1), V1 > V).
